...
def classify_text():
    try:
        ...
        thread = threading.Thread(
            target=process_classification, args=(user_id, upload_history_id))
        thread.start()
...
def process_classification(user_id, upload_history_id):
    with app.app_context():
        try:
            expense_classifier = ExpenseClassifier(
                user_id, upload_history_id, classifier)
            expense_classifier.process_table_data()
            print(
                f"Classification completed for user {user_id}, upload {upload_history_id}")
        except Exception as e:
