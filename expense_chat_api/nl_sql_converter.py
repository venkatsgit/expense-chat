...
from few_shot_manager import FewShotManager  # Import the FewShotManager class from your module
...
class NL2SQLConverter:
    ...
    def __init__(
        ...
        self.few_shot_manager = FewShotManager(db_type=db_type)
        ...
        if self.table_info:
            ...
            for table_name, columns in self.table_info.items():
                ...
                if isinstance(columns, list):
                    table_info_text += "- Columns: " + ", ".join([f"`{col}`" for col in columns]) + "\n"
...
if __name__ == "__main__":
    ...
    for query in test_queries:
        print(f"\nQuery: {query}")
        result = converter.convert_to_sql(query, user_id=123)
